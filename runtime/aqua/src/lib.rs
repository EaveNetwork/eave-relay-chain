...
//! The Aqua runtime for v1 parachains.
...
construct_runtime! {
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		// runtime as shared by both the Aqua and Wococo chains. When running as Aqua we only use
		// `BridgeWococoGrandpa`, and vice versa.
		BridgeAquaGrandpa: pallet_bridge_grandpa::{Pallet, Call, Storage, Config<T>} = 40,
...
pub type SovereignAccountOf = (
	...
	AccountId32Aliases<AquaNetwork, AccountId>,
);
...
type LocalOriginConverter = (
	...
	SignedAccountId32AsNative<AquaNetwork, Origin>,
...
parameter_types! {
	pub const AquaForTick: (MultiAsset, MultiLocation) =
		(AllConcreteFungible { id: Null }, X1(Parachain(100)));
	pub const AquaForTrick: (MultiAsset, MultiLocation) =
		(AllConcreteFungible { id: Null }, X1(Parachain(110)));
	pub const AquaForTrack: (MultiAsset, MultiLocation) =
		(AllConcreteFungible { id: Null }, X1(Parachain(120)));
	pub const AquaForStatemint: (MultiAsset, MultiLocation) =
		(AllConcreteFungible { id: Null }, X1(Parachain(1001)));
}
pub type TrustedTeleporters = (
	xcm_builder::Case<AquaForTick>,
	xcm_builder::Case<AquaForTrick>,
	xcm_builder::Case<AquaForTrack>,
	xcm_builder::Case<AquaForStatemint>,
);
...
pub type LocalOriginToLocation = (
	...
	SignedToAccountId32<Origin, AccountId, AquaNetwork>,
);
...
pub type AquaGrandpaInstance = ();
impl pallet_bridge_grandpa::Config for Runtime {
	type BridgedChain = bp_aqua::Aqua;
...
sp_api::impl_runtime_apis! {
	...
	impl bp_aqua::AquaFinalityApi<Block> for Runtime {
		fn best_finalized() -> (bp_aqua::BlockNumber, bp_aqua::Hash) {
			let header = BridgeAquaGrandpa::best_finalized();
			(header.number, header.hash())
		}

		fn is_known_header(hash: bp_aqua::Hash) -> bool {
			BridgeAquaGrandpa::is_known_header(hash)
		}
	}
